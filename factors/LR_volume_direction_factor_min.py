...
def Lee_Ready_Direction(df_tick):
    ...
    df_tick['Buy_Volume'] = df_tick['volume'] * df_tick['Buy_Direction']
    df_tick['Sell_Volume'] = df_tick['volume'] * df_tick['Sell_Direction']
    return df_tick


def LR_Volume_Direction_min(df_min,df_tick):
    df_tick = Lee_Ready_Direction(df_tick)
    # sum for minute
    buy_volume = df_tick.groupby(['min_time', 'trading_date'])['Buy_Volume'].sum().to_dict()
    sell_volume = df_tick.groupby(['min_time', 'trading_date'])['Sell_Volume'].sum().to_dict()
    df_min['Buy_Volume'] = df_min.apply(lambda x: buy_volume.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['Sell_Volume'] = df_min.apply(lambda x: sell_volume.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['LR_volume_min'] = (df_min['Buy_Volume'])/(df_min['Buy_Volume']+df_min['Sell_Volume'])
    df_min['LR_volume_min'] = df_min['LR_volume_min'].ffill()
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = LR_Volume_Direction_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['LR_volume_min'])
    filtered_df = filtered_df.drop(columns='LR_volume_min')
    ...
    factor_df = pd.DataFrame(merged_df['LR_volume_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'LR_volume_min.parquet')
