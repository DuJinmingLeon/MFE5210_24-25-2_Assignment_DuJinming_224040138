...
def cal_excess_vol_tick(df_tick):
    df_tick['OI'] = df_tick['BVC_buy_vol'] - df_tick['BVC_sell_vol']
    mask_1 = (df_tick['OI'] > 0)
    df_tick['excess_vol_signal'] = 0
    df_tick.loc[mask_1, 'excess_vol_signal'] = 1
    return df_tick


def excess_vol_duration_min(df_min,df_tick):
    df_tick = cal_BVC_vol(df_tick)
    df_tick = cal_excess_vol_tick(df_tick)
    excess_vol_signal_sum = df_tick.groupby(['min_time', 'trading_date'])['excess_vol_signal'].sum().to_dict()
    df_min['BVC_EVD_min'] = df_min.apply(lambda x: excess_vol_signal_sum.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['BVC_EVD_min'] = df_min['BVC_EVD_min']/120
    df_min['BVC_EVD_min'] = df_min['BVC_EVD_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = excess_vol_duration_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['BVC_EVD_min'])
    filtered_df = filtered_df.drop(columns='BVC_EVD_min')
    ...
    factor_df = pd.DataFrame(merged_df['BVC_EVD_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'BVC_EVD_min.parquet')
