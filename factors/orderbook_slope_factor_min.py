...
def calculate_ols_slope(df, direction):
    x_cols = [direction+f'CumuVolume{i}' for i in range(1, 6)]
    y_cols = [direction+f'{i}' for i in range(1, 6)]
    ...
    slopes = beta[:, 1, 0]
    #df['Slope'] = pd.Series(slopes, index=df.index)
    #return df['Slope'].values
    return pd.Series(slopes, index=df.index)


def slope_tick(df_tick):
    df_tick['Buy_Slope'] = np.nan
    df_tick['Sell_Slope'] = np.nan
    df_tick = calculate_cumulative_volume(df_tick,'b')
    df_tick = calculate_cumulative_volume(df_tick,'a')
    df_tick['Buy_Slope'] = calculate_ols_slope(df_tick,'b')
    df_tick['Sell_Slope'] = calculate_ols_slope(df_tick,'a')
    df_tick['Buy_Slope'] = df_tick['Buy_Slope'].ffill()
    df_tick['Sell_Slope'] = df_tick['Sell_Slope'].ffill()
    df_tick['fac_tick'] = df_tick['Buy_Slope'] - df_tick['Sell_Slope']
    return df_tick


def cal_orderbook_slope_min(df_min, df_tick):
    df_tick = slope_tick(df_tick)
    # sum for minute
    slope_mean = df_tick.groupby(['min_time', 'trading_date'])['fac_tick'].mean().to_dict()
    df_min['orderbook_slope_min'] = df_min.apply(lambda x: slope_mean.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['orderbook_slope_min'] = df_min['orderbook_slope_min'].ffill()
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_orderbook_slope_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['orderbook_slope_min'])
    factor_df['orderbook_slope_min'] = -1*factor_df['orderbook_slope_min']
    filtered_df = filtered_df.drop(columns='orderbook_slope_min')
    ...
    factor_df = pd.DataFrame(merged_df['orderbook_slope_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'orderbook_slope_min.parquet')
