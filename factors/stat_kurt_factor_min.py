...
def cal_tick_rtn(df_tick):
    ...
    #df_tick['TradeAmount'] = df_tick['mid_price'] * df_tick['TradeVolume']
    ...
    df_tick['Rtn_tick'] = df_tick['Rtn_tick'].ffill()

    return df_tick


def cal_kurt_min(df_min,df_tick):
    ...
    # 计算每个分组的偏度
    result = grouped.agg(
        ret_kurt_min=('Rtn_tick', lambda x: x.kurt())
    ).reset_index()

    df_min = df_min.merge(result, on=['trading_date', 'min_time'], how='left')
    df_min['ret_kurt_min'] = (df_min['ret_kurt_min']-3)#/(df_min['Amount_M01']/df_min['Volume_M01'])
    df_min['ret_kurt_min'] = df_min['ret_kurt_min'].apply(np.log)
    df_min['ret_kurt_min'] = df_min['ret_kurt_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_kurt_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['ret_kurt_min'])
    filtered_df = filtered_df.drop(columns='ret_kurt_min')
    ...
    factor_df = pd.DataFrame(merged_df['ret_kurt_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'ret_kurt_min.parquet')
