...
def cal_tick_rtn(df_tick):
    ...
    # 最新成交价版本
    #df_tick['Rtn_tick'] = (df_tick['LastPrice'] / df_tick['LastPrice'].shift(1)) - 1
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_skew_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['ret_skew_min'])
    factor_df['ret_skew_min'] = -1*factor_df['ret_skew_min']
    filtered_df = filtered_df.drop(columns='ret_skew_min')
    ...
    factor_df = pd.DataFrame(merged_df['ret_skew_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'ret_skew_min.parquet')
