...
def cal_tick_return(df_tick):
    ...
    df_tick['log_return'] = df_tick['log_return'].fillna(0)
    df_tick['return_squared'] = df_tick['log_return']*df_tick['log_return'].shift(1)
    df_tick['return_squared'] = df_tick['return_squared'].fillna(0)
    return df_tick


def bv_min(df_min,df_tick):
    ...
    df_min['down1_min'] = df_min['down_min'] - 1
    df_min['bv_min'] = df_min['up_min']*(df_min['down_min']/df_min['down1_min'])
    df_min['bv_min'] = df_min['bv_min'].ffill()
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = bv_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['bv_min'])

    #factor_df['bv_min'] = -1*factor_df['bv_min']

    filtered_df = filtered_df.drop(columns='bv_min')
