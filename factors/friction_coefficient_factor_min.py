...
def calc_illiq_fraction(row):
    ...
    if row['close'] >= row['previous_close']:
        ...
        dr = (row['volume'] ** 2) / (row['open_interest'] ** 2) * (
                    np.abs((row['high'] - row['previous_close']) / (row['previous_close'] + 0.001)) + np.abs(
                (row['low'] - row['high']) / (row['high'] + 0.001)) + np.abs(
                (row['close'] - row['low']) / (row['low'] + 0.001)))
        fac_value = up / dr
    elif row['close'] < row['previous_close']:

        dr = -1 * (row['volume'] ** 2) / ((row['open_interest'] ** 2) * (
                    np.abs((row['low'] - row['previous_close']) / (row['previous_close'] + 0.001)) + np.abs(
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df['friction_coefficient_min'] = fraction_coefficient_min(filtered_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['friction_coefficient_min'])
    filtered_df = filtered_df.drop(columns='friction_coefficient_min')
    ...
    factor_df = pd.DataFrame(merged_df['friction_coefficient_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'friction_coefficient_min.parquet')
