...
def cal_tick_spread_amount(df_tick):
    ...
    df_tick['spread_tick'] = (df_tick['a1'] - df_tick['b1']) / df_tick['mid_price']
    #df_tick['TradeAmount'] = df_tick['volume'] * df_tick['last']
    #df_tick['up'] = df_tick['spread_tick'] * df_tick['TradeVolume']
    df_tick['up'] = df_tick['spread_tick']*df_tick['total_turnover']
    return df_tick


def cal_spread_bidask_min(df_min,df_tick):
    df_tick = cal_tick_spread_amount(df_tick)
    fac_tick_sum = df_tick.groupby(['min_time', 'trading_date'])['up'].sum().to_dict()
    #amount_tick_sum_M01 = df_tick_M01.groupby(['min_time', 'TradingDate'])['TradeVolume'].sum().to_dict()
    df_min['spread_bidask_min'] = df_min.apply(lambda x: fac_tick_sum.get((x['min_time'], x['trading_date']), 0), axis=1)
    #df_min['FactorDOWN_M01'] = df_min.apply(lambda x: amount_tick_sum_M01.get((x['min_time'], x['TradingDate']), 0), axis=1)
    df_min['spread_bidask_min'] = df_min['spread_bidask_min']/df_min['total_turnover']
    df_min['spread_bidask_min'] = df_min['spread_bidask_min'].ffill()
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_spread_bidask_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['spread_bidask_min'])
    filtered_df = filtered_df.drop(columns='spread_bidask_min')
    ...
    factor_df = pd.DataFrame(merged_df['spread_bidask_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'spread_bidask_min.parquet')
