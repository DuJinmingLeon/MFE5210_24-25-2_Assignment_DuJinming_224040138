...
def cal_BVC_vol(df_tick):
    ...
    df_tick['OFI'] = df_tick['sell_vol'] - df_tick['buy_vol']
    return df_tick


def cal_OFIV_min(df_min,df_tick):
    df_tick = cal_BVC_vol(df_tick)
    OFI_std = df_tick.groupby(['min_time', 'trading_date'])['OFI'].std().to_dict()
    df_min['OFIV_min'] = df_min.apply(lambda x: OFI_std.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['OFIV_min'] = df_min['OFIV_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_OFIV_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['OFIV_min'])
    filtered_df = filtered_df.drop(columns='OFIV_min')
    ...
    factor_df = pd.DataFrame(merged_df['OFIV_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'OFIV_min.parquet')
