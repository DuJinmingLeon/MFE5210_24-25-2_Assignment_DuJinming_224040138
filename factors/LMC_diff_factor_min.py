...
def calculate_ols_slope(df,flag):
    x_cols = [flag + f'CumuVolume{i}' for i in range(1, 6)]
    y_cols = [flag + f'AvgCost{i}' for i in range(1, 6)]
...
def slope_tick(df_tick):
    ...
    #df_tick['fac_tick'] = df_tick['Buy_Slope']/(df_tick['b1']**2) - df_tick['Sell_Slope']/(df_tick['a1']**2)

    df_tick['Slope'] = df_tick['Buy_Slope'] - df_tick['Sell_Slope']
    df_tick['Slope'] = df_tick['Slope'].ffill()
    df_tick['fac_tick'] = df_tick['Slope'] / (((df_tick['b1'] + df_tick['a1']) / 2) ** 2)

    return df_tick


def LMC_diff_min(df_min, df_tick):
    df_tick = slope_tick(df_tick)
    # sum for minute
    slope_mean = df_tick.groupby(['min_time', 'trading_date'])['fac_tick'].mean().to_dict()
    df_min['LMC_diff_min'] = df_min.apply(lambda x: slope_mean.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['LMC_diff_min'] = df_min['LMC_diff_min'].ffill()
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = LMC_diff_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['LMC_diff_min'])
    filtered_df = filtered_df.drop(columns='LMC_diff_min')
    ...
    factor_df = pd.DataFrame(merged_df['LMC_diff_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'LMC_diff_min.parquet')
    ...
    #print("\nNet Asset Value (NAV):")
    #print(nav)
    #nav.to_csv('E:\\Internship\\galaxy_commodity\\factors\\amihud_illiq_factor_min\\amihud_min_factor_daily_nav.csv')\
    backtester.plot_nav()
