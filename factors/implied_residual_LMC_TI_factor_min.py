...
def IRLMC_min(df_min, df_tick):
    ...
    df_min['implied_resid_TI_min'] = df_min.apply(lambda x: resid_sum.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['implied_resid_TI_min'] = df_min['implied_resid_TI_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    factor_df = pd.DataFrame(filtered_df['implied_resid_TI_min'])
    factor_df['implied_resid_TI_min'] = -1 * factor_df['implied_resid_TI_min']
    filtered_df = filtered_df.drop(columns='implied_resid_TI_min')
    ...
    factor_df = pd.DataFrame(merged_df['implied_resid_TI_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    #factor_df.to_parquet(factor_store_path + 'implied_resid_TI_min.parquet')
