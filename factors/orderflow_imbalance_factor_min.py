...
def OFI_min(df_min, df_tick):
    ...
    # sum for minute
    ofi_sum = df_tick.groupby(['min_time', 'trading_date'])['OFI'].sum().to_dict()

    df_min['OFI_min'] = df_min.apply(lambda x: ofi_sum.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['OFI_min'] = df_min['OFI_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = OFI_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['OFI_min'])
    filtered_df = filtered_df.drop(columns='OFI_min')
    ...
    factor_df = pd.DataFrame(merged_df['OFI_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'OFI_min.parquet')
