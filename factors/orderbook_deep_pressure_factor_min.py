...
def cal_pressure(df_tick):
    ...
    for dlevel in deep_book_lst:
        df_tick['BuyPressure'+dlevel] = (1/(df_tick['b'+dlevel]-df_tick['S0']) + (df_tick['gamma']*(0.05**(1/3)))/((df_tick['b'+dlevel]-df_tick['S0'])**2) + ((df_tick['gamma']*(0.05**(1/3)))**2)/((df_tick['b'+dlevel]-df_tick['S0'])**3))/1e10
        df_tick['SellPressure'+dlevel] = (1/(df_tick['a'+dlevel]-df_tick['S0']) + (
                    df_tick['gamma']*(0.05**(1/3)))/((df_tick['a'+dlevel]-df_tick['S0'])**2) + (
                                                      (df_tick['gamma']*(0.05 ** (1 / 3)))**2)/(
                                                      (df_tick['a'+dlevel]-df_tick['S0'])**3))/1e10
    for i in range(3, 6):
    ...
    df_tick['BuyPressure'] = (df_tick['BuyVolPre3']+df_tick['BuyVolPre4']+df_tick['BuyVolPre5'])/(df_tick['b3_v']+df_tick['b4_v']+df_tick['b5_v'])
    df_tick['SellPressure'] = (df_tick['SellVolPre3']+df_tick['SellVolPre4']+df_tick['SellVolPre5'])/(df_tick['a3_v']+df_tick['a4_v']+df_tick['a5_v'])
...
def cal_order_flow_pressure_min(df_min, df_tick):
    ...
    df_min['deep_pressure_min'] = df_min.apply(lambda x: pressure_mean.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['deep_pressure_min'] = df_min['deep_pressure_min'].ffill()
    df_min['deep_pressure_min'] = -1 * df_min['deep_pressure_min']
    return df_min
...
if __name__ == '__main__':
    ...
    factor_df = pd.DataFrame(filtered_df['deep_pressure_min'])
    filtered_df = filtered_df.drop(columns='deep_pressure_min')
    ...
    factor_df = pd.DataFrame(merged_df['deep_pressure_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'deep_pressure_min.parquet')
