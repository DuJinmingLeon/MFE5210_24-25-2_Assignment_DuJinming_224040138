...
def cal_tick_rtn(df_tick):
    ...
    #df_tick['Rtn_tick'] = (df_tick['mid_price'] / df_tick['mid_price'].shift(1)) - 1
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = cal_semi_stddev_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['semi_stddev_min'])
    filtered_df = filtered_df.drop(columns='semi_stddev_min')
    ...
    factor_df = pd.DataFrame(merged_df['semi_stddev_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'semi_stddev_min.parquet')
