...
def MLOFI_min(df_min, df_tick):
    # 5档
    for i in range(5):
        df_tick['delta_BidVolume' + str(i + 1)] = np.where(
            df_tick['b' + str(i + 1)] == df_tick['b' + str(i + 1)].shift(1),
            df_tick['b' + str(i + 1) + '_v'] - df_tick['b' + str(i + 1) + '_v'].shift(1),
            np.where(df_tick['b' + str(i + 1)] > df_tick['b' + str(i + 1)].shift(1),
                     df_tick['b' + str(i + 1) + '_v'],
                     -1 * df_tick['b' + str(i + 1) + '_v'].shift(1)))

        df_tick['delta_AskVolume' + str(i + 1)] = np.where(
            df_tick['a' + str(i + 1)] == df_tick['a' + str(i + 1)].shift(1),
            df_tick['a' + str(i + 1) + '_v'] - df_tick['a' + str(i + 1) + '_v'].shift(1),
            np.where(df_tick['a' + str(i + 1)] > df_tick['a' + str(i + 1)].shift(1),
                     -1 * df_tick['a' + str(i + 1) + '_v'].shift(1),
                     df_tick['a' + str(i + 1) + '_v']))

    df_tick['MLOFI'] = df_tick[[('delta_BidVolume' + str(j + 1)) for j in range(5)]].sum(axis=1) - df_tick[
        [('delta_AskVolume' + str(j + 1)) for j in range(5)]].sum(axis=1)
    df_tick['MLOFI'] = df_tick['MLOFI'].ffill()

    # sum for minute
    mlofi_sum = df_tick.groupby(['min_time', 'trading_date'])['MLOFI'].sum().to_dict()
    df_min['MLOFI_min'] = df_min.apply(lambda x: mlofi_sum.get((x['min_time'], x['trading_date']), 0), axis=1)
    df_min['MLOFI_min'] = df_min['MLOFI_min'].ffill()
    return df_min
...
if __name__ == '__main__':
    ...
    # 因子计算
    filtered_df = MLOFI_min(filtered_df,tick_df)
    filtered_df = filtered_df.dropna()
    # 检查是否含有 inf 或 -inf
    has_inf = filtered_df.isin([np.inf, -np.inf]).any(axis=1)
    # 删除含有 inf 或 -inf 的行
    filtered_df = filtered_df[~has_inf]
    factor_df = pd.DataFrame(filtered_df['MLOFI_min'])
    filtered_df = filtered_df.drop(columns='MLOFI_min')
    ...
    factor_df = pd.DataFrame(merged_df['MLOFI_min'])
    print(factor_df)
    factor_store_path = 'E:\\Schoolwork Files\\MScFE\\MFE5210_Algo_Trading\\Assignment\\data\\factor_data\\AG\\'
    factor_df.to_parquet(factor_store_path + 'MLOFI_min.parquet')
